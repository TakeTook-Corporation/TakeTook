package com.example.core.widgets.hiddensearchrecycler
...
import com.example.core.widgets.R
...
class HiddenSearchWithRecyclerView @JvmOverloads constructor(
    ...
    private var hideAtScroll = true
    private var scrollToTopBeforeShow = false
    private var scrollToBottomBeforeHide = false
    ...
    private fun onActionUp(event: MotionEvent) {
        if (abs(startYDrag - lastYDrag) > convertDpToPixel(context, MIN_SCROLL_TO_ANIM)) {
            ...
            if (abs(startYDrag - lastYDrag) > convertDpToPixel(context, MIN_TAP_MOVEMENT) && !searchBarSearchView.hasFocus()) {
                if (movementDirection == Movement.DOWN) {
                    moveSearchToVisible()
                } else {
                    moveSearchBarToHide()
    ...
    private fun onActionMove(event: MotionEvent) {
        ...
        if (delta > 0) {
            if (searchBarCanMoveDown && checkRecyclerNeedToScroll(movementDirection)) {
                dragSearch(delta)
            } else {
                dispatchEventToChildView(event)
            }
        } else {
            if (searchBarCanMoveUp && checkRecyclerNeedToScroll(movementDirection) && hideAtScroll) {
                dragSearch(delta)
            } else {
                dispatchEventToChildView(event)
            }
    ...
    private fun checkRecyclerNeedToScrollOnDownMovement(firstElement: Int?): Boolean {
        return if (scrollToTopBeforeShow) firstElement == 0 else true
    }
    ...
    private fun moveSearchToVisible() {
        ...
        val newDuration = if (totalMovement != 0 && remainingMovement != 0) ANIM_DURATION / totalMovement * remainingMovement else 0

        with(modalAnimator) {
            duration = if (newDuration > 0) newDuration else 0
            addUpdateListener {
    ...
    private fun parseStyleAttrs(attributes: AttributeSet?) {
        val attrs = context.theme.obtainStyledAttributes(attributes, R.styleable.HiddenSearchWithRecyclerView, 0, 0)

        try {
            visibleAtInit =
                attrs.getBoolean(R.styleable.HiddenSearchWithRecyclerView_visibleAtInit, false)
            hideAtScroll = attrs.getBoolean(R.styleable.HiddenSearchWithRecyclerView_hideAtScroll, true)
            scrollToBottomBeforeHide = attrs.getBoolean(
                R.styleable.HiddenSearchWithRecyclerView_scrollToBottomBeforeHide,
                false
            )
            scrollToTopBeforeShow = attrs.getBoolean(R.styleable.HiddenSearchWithRecyclerView_scrollToTopBeforeShow, false)
            filterWhileTyping = attrs.getBoolean(R.styleable.HiddenSearchWithRecyclerView_filterWhileTyping, true)
        } finally {
            attrs.recycle()
        }
    }
